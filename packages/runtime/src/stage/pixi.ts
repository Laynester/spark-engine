...
export class PixiStage implements StageAdapter {
  ...
  private hitTest(x: number, y: number, opts?: { onlyScripted?: boolean }): number {
    ...
    for (const hit of hits) {
      ...
      // click aimed at the avatar, while a composited panel whose ink cannot key
      // keeps its rectangle. Everything else keeps Director's rectangle rule.
      const srcImg = hit.node.imgLImage ?? ch.member?.image;
      const alphaArt = (srcImg?.depth ?? 0) >= 32;
      if (spritePixelHitTest(ch.ink ?? 0, hit.node.imgBuffer, sw, sh, px, py, alphaArt)) return hit.channel;
    }
